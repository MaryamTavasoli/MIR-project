...
class Index:
    ...
    def index_stars(self):
        ...
        for doc in self.preprocessed_documents:
            for star in doc.get('stars', []):
                current_index.setdefault(star, {}).setdefault(doc['id'], 0)
    ...
    def index_genres(self):
        ...
        for doc in self.preprocessed_documents:
            for genre in doc.get('genres', []):
                current_index.setdefault(genre, {}).setdefault(doc['id'], 0)
    ...
    def index_summaries(self):
        ...
        for doc in self.preprocessed_documents:
            for summary in doc.get('summaries', []):
                current_index.setdefault(summary, {}).setdefault(doc['id'], 0)
                current_index[summary][doc['id']] += 1
        return current_index
    ...
    def add_document_to_index(self, document: dict):
        ...
        # Indexing in documents index
        self.index[Indexes.DOCUMENTS.value][doc_id] = document

        # Indexing in stars
        for star in document.get('stars', []):
            self.index[Indexes.STARS.value].setdefault(star, {}).setdefault(doc_id, 0)
            self.index[Indexes.STARS.value][star][doc_id] += 1

        # Indexing in genres
        for genre in document.get('genres', []):
            self.index[Indexes.GENRES.value].setdefault(genre, {}).setdefault(doc_id, 0)
            self.index[Indexes.GENRES.value][genre][doc_id] += 1

        # Indexing in summaries
        for summary in document.get('summaries', []):
            self.index[Indexes.SUMMARIES.value].setdefault(summary, {}).setdefault(doc_id, 0)
            self.index[Indexes.SUMMARIES.value][summary][doc_id] += 1
    ...
    def remove_document_from_index(self, document_id: str):
        ...
        #         TODO
        # Removing from documents index
        self.index[Indexes.DOCUMENTS.value].pop(document_id, None)

        # Removing from stars index
        for star in self.index[Indexes.STARS.value]:
            self.index[Indexes.STARS.value][star].pop(document_id, None)

        # Removing from genres index
        for genre in self.index[Indexes.GENRES.value]:
            self.index[Indexes.GENRES.value][genre].pop(document_id, None)

        # Removing from summaries index
        for summary in self.index[Indexes.SUMMARIES.value]:
            self.index[Indexes.SUMMARIES.value][summary].pop(document_id, None)
    ...
    def store_index(self, path: str, index_name: str = None):
        ...
        if index_name is None:
            ...
            filename = "entire_index.json"

        if index_name not in self.index:
            raise ValueError('Invalid index type')
        else:
            index_to_store = {index_name: self.index[index_name]}
            filename = f"{index_name}.json"

        #         TODO
        with open(os.path.join(path, filename), 'w') as file:
            json.dump(index_to_store, file, indent=4)
