...
class SearchEngine:
        ...
        for field in weights:
            for tier in ["first_tier", "second_tier", "third_tier"]:
                ...
                if method == 'OkapiBM25':
                    ...
                    if tier == "first_tier":
                        scores1[field] = scorer.compute_socres_with_okapi_bm25(query, average_document_length,
                                                                               document_length)
                    elif tier == "second_tier":
                        scores2[field] = scorer.compute_socres_with_okapi_bm25(query, average_document_length,
                                                                               document_length)
                    else:
                        scores3[field] = scorer.compute_socres_with_okapi_bm25(query, average_document_length,
                                                                               document_length)
                else:
                    if tier == "first_tear":
                        scores1[field] = scorer.compute_scores_with_vector_space_model(query, method)
                    elif tier == "second_tear":
                        scores2[field] = scorer.compute_scores_with_vector_space_model(query, method)
                    else:
                        scores3[field] = scorer.compute_scores_with_vector_space_model(query, method)
                if scores1 and scores2:
                    scores = self.merge_scores(scores1, scores2, field)
                if scores and scores3:
    ...
    def find_scores_with_safe_ranking(self, query, method, weights, scores):
        ...
        for field in weights:
            # TODO
            field1 = ""
            if field == Indexes.STARS:
                field1 = 'stars'
            elif field == Indexes.GENRES:
                field1 = 'genres'
            else:
                field1 = 'summaries'
            scorer = Scorer(self.document_indexes[field].index[field1], self.metadata_index.index['document_count'])
            if method == 'OkapiBM25':
                average_document_length = self.metadata_index["average_document_length"][field.value]
                document_length = self.document_lengths_index[field]
                scores[field] = scorer.compute_socres_with_okapi_bm25(query, average_document_length, document_length)
            else:
    ...
    def find_scores_with_unigram_model(
        ...
        for field in weights:
            ...
            scorer = Scorer(self.document_indexes[field].index[field1], self.metadata_index.index['document_count'])
            document_length = self.document_lengths_index[field]
            scores[field] = scorer.compute_scores_with_unigram_model(query,smoothing_method,document_length,alpha,lamda)
