...
with open('/Users/maryamtavasoli/Desktop/IMDb-IR-System-main-phase2/Logic/core/indexes/documents_index.json',
          'r') as f3:
    movies_dataset = json.load(f3)
movies_dataset=movies_dataset['documents']
all_documents = []
for movie_id, movie_details in movies_dataset.items():
    all_documents.append(movie_details.get("title", ""))
    all_documents.append(movie_details.get("first_page_summary", ""))
    if movie_details.get("stars") is not None:
        all_documents.append(" ".join(movie_details.get("stars", "")))
    if movie_details.get("geners") is not None:
        all_documents.append(" ".join(movie_details.get("geners", "")))
...
def get_movie_by_id(id: str, movies_dataset: List[Dict[str, str]]) -> Dict[str, str]:
    ...
    images_folder = "/Users/maryamtavasoli/Desktop/IMDb-IR-System-main-phase2/Logic/core/Images"  # Replace with your actual path

    if os.path.exists(os.path.join(images_folder, image_filename)):
        result["Image_URL"] = f"/Users/maryamtavasoli/Desktop/IMDb-IR-System-main-phase2/Logic/core/Images/{image_filename}"  # URL to your images folder
    else:
        result["Image_URL"] = (
